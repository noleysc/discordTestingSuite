...
public class ServerSettingsPage extends BasePage {
    ...
    public void enableAdministratorPermission() {
        ...
        WebElement permissionsTab = (WebElement) ((JavascriptExecutor) driver).executeScript(
        ...
        if (searchInput != null) {
            ...
            simulateThinking(800, 1500);
        }

        // 3. Toggle the Administrator switch
        WebElement adminToggle = (WebElement) ((JavascriptExecutor) driver).executeScript(
            "return Array.from(document.querySelectorAll('span')).find(el => el.textContent.trim() === 'Administrator')" +
            ".closest('[class*=\"container\"]').querySelector('[role=\"switch\"], input[type=\"checkbox\"]');"
        );
        ...
        if (adminToggle != null) {
            ...
            if ("false".equals(state)) {
                ...
                logger.info("Administrator permission already enabled.");
            }
        }
