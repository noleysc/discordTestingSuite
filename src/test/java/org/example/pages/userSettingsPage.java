...
public class userSettingsPage extends basePage {
    ...
    public void editAboutMe(String bio) {
        if (bio == null) return;
        logger.info("Editing About Me (Bio) section...");
        WebElement bioInput = (WebElement) ((JavascriptExecutor) driver).executeScript(
            "const content = document.querySelector('[class*=\"content\"]') || document.querySelector('main');" +
            "return content.querySelector('textarea[id*=\"bio\"]') || " +
            "       content.querySelector('[aria-label*=\"About Me\"]') || " +
            "       content.querySelector('div[role=\"textbox\"][data-slate-editor=\"true\"]');"
        );

        if (bioInput != null) {
            clickHumanly(bioInput);
            bioInput.sendKeys(Keys.chord(getModifierKey(), "a"), Keys.BACK_SPACE);
            typeHumanly(bioInput, bio);
            ((JavascriptExecutor) driver).executeScript("arguments[0].dispatchEvent(new Event('input', { bubbles: true }));", bioInput);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            try {
                ...
                if (errorText != null) {
                    ...
                    Thread.sleep(60000);
                    // Dismiss the error if possible or just try clicking save again
                }
            ...
            if (saveBtn == null) {
                // Try specifically targeting the notice bar button
                saveBtn = (WebElement) ((JavascriptExecutor) driver).executeScript(
    ...
    public String getInputValue(String label) {
        String script = 
            ...
            "// Strategy 1: Find input by name or common attributes directly\n" +
            ...
            "}\n" +
            "// Strategy 2: Find label element and look for associated input\n" +
            ...
            "// Fallback: search following siblings\n" +
